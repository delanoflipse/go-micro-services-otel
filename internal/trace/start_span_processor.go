...
type directSpanProcessor struct {
	exporterMu sync.Mutex
	exporter   otelTrace.SpanExporter
	stopOnce   sync.Once
}

var _ otelTrace.SpanProcessor = (*directSpanProcessor)(nil)
...
// use instead.
func NewDirectSpanProcessor(exporter otelTrace.SpanExporter) otelTrace.SpanProcessor {
	ssp := &directSpanProcessor{
		exporter: exporter,
	}
...
func (ssp *directSpanProcessor) OnStart(c context.Context, s otelTrace.ReadWriteSpan) {
	...
	if ssp.exporter != nil && s.SpanContext().TraceFlags().IsSampled() {
		if err := ssp.exporter.ExportSpans(context.Background(), []otelTrace.ReadOnlySpan{s}); err != nil {
			otel.Handle(err)
		}
	}
}
...
func (ssp *directSpanProcessor) OnEnd(s otelTrace.ReadOnlySpan) {
	ssp.exporterMu.Lock()
	defer ssp.exporterMu.Unlock()
...
func (ssp *directSpanProcessor) Shutdown(ctx context.Context) error {
	var err error
	ssp.stopOnce.Do(func() {
		stopFunc := func(exp otelTrace.SpanExporter) (<-chan error, func()) {
...
func (ssp *directSpanProcessor) ForceFlush(context.Context) error {
	return nil
}

// MarshalLog is the marshaling function used by the logging system to represent
// this Span Processor.
func (ssp *directSpanProcessor) MarshalLog() interface{} {
	return struct {
		Type     string
		Exporter otelTrace.SpanExporter
	}{
